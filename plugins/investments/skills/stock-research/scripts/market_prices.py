...
def alpaca_actions(client, args):
    ...
    requested_groups = {kind + "s" for kind in requested_types}
    ...
    def decode(payload):
        ...
        for kind, rows in groups.items():
            ...
            # The response groups pluralize the singular query types. Providers
            # may include empty groups; a nonempty unrequested group is a scope
            # mismatch, not evidence for the requested split/dividend event.
            if rows and requested_groups and kind not in requested_groups:
                raise DataError("invalid_response", "Alpaca returned a corporate-action type outside the query.")
            for raw in rows:
...
def run_self_test():
    ...
    class Cases(unittest.TestCase):
        ...
        def test_actions_reject_nonempty_unrequested_types(self):
            client = FakeClient([action_page([action()], kind="cash_dividends")])
            with self.assertRaisesRegex(DataError, "type outside the query"):
                alpaca_actions(client, args(start="2026-09-01", end="2026-09-05",
                                           types="forward_split"))

        def test_actions_later_type_mismatch_keeps_validated_earlier_events(self):
            client = FakeClient([
                action_page([action()], "next", kind="forward_splits"),
                action_page([action(id="dividend")], kind="cash_dividends"),
            ])
            result = alpaca_actions(client, args(start="2026-09-01", end="2026-09-05",
                                                types="forward_split"))
            self.assertFalse(result["complete"])
            self.assertEqual([item["type"] for item in result["data"]["actions"]],
                             ["forward_splits"])
            self.assertEqual(result["pagination"]["pages"], 1)

        def test_actions_allow_empty_unrequested_groups_and_multiple_filters(self):
            payload = {"corporate_actions": {
                "forward_splits": [action()],
                "cash_dividends": [action(id="dividend")],
                "name_changes": [],
            }, "next_page_token": None}
            result = alpaca_actions(FakeClient([payload]), args(
                start="2026-09-01", end="2026-09-05", types="forward_split,cash_dividend"))
            self.assertTrue(result["complete"])
            self.assertEqual({item["type"] for item in result["data"]["actions"]},
                             {"forward_splits", "cash_dividends"})
