...
it does not independently rename unrelated notes or images. Investment
records in vault-root `Investments/` remain unchanged, including through a
linked-folder alias. If one has a live dependency that the rename would
change, the helper blocks the entire rename; retain the source's current name
and location. General authorization to repair source references does not make
these historical records editable. Unrelated records and literal examples do
not block a rename.
