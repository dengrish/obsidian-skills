# Filename and image slug rules (step 3)
...
**Every example below is a `--topic` example, and that is the documented path** (SKILL.md step 3): *you* pick the 2–4 content words, the script does the mechanics. `slug.py --title '<full title>'` also exists, but it is a **suggestion to check, not a shortcut** — it drops a fixed filler list and keeps the first few words left in order, so a title whose topic isn't simply "its first content words" comes out wrong. Seven of the eight examples on this page do not reproduce from it: `"Pancreatic cancer just met its match"` yields `Pancreatic_Cancer_Match`, not `Pancreatic_Cancer`; `"How LLMs work: a deep dive"` yields `LLMs_Work_Deep_Dive`, not `LLMs_Deep_Dive`. The script says so itself — a `--title` run sets `topic_auto: true` and lists what it kept and dropped in `notes`. **The slug is also the image-filename prefix**, so accepting an unchecked `--title` answer and correcting the name afterwards files that clipping's figures under a stem nothing looks for. Read the `notes`, then re-run with `--topic` if the topic isn't right.
...
**Author segment** — the surname of the **first author**. Resolve it in two stages so commas don't trip you up:

1. **Pick the first author.** Authors normally arrive as a YAML list (one entry per author from step 2), so the first author is simply the first list entry. If instead the authors are mashed into a *single string* — multiple names joined by `and`, `&`, `;`, or a comma-separated list like `Buck, Carlsmith, and Greenblatt` — take only the portion before the first such separator as the first author, and ignore the rest.
...
**Year segment** — 4-digit year from the **corrected** `published` date. If `published` couldn't be verified and isn't in the raw, fall back to the year from `created` (the clipping date). If even that's somehow missing, drop the year segment.
...
Use the **corrected** author and title from step 2 (not the raw), so the slug reflects the verified metadata — this is why getting source verification right matters for findability, not just for the YAML.
