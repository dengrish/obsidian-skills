...
3. It detects the actual file type **from the bytes**, not from the URL's extension and not from the `Content-Type` (Substack's `image/fetch` URLs end in `.png` but often serve webp; many CDNs do similar). The order is evidence first, claims second:
   ...
   - **Unrecognized bytes fail too.** An `image/*` header or a `.png` URL does not make arbitrary binary data an image. A format without a recognized signature needs a supported conversion at a scratch path, not a guessed extension.
   ...
   - Signature detection identifies the format; it does not fully decode the image. Still open the finished file to check for corruption and readability before embedding it.
4. It publishes the complete file as `<vault>/Sources/Images/<image_slug>_fig_<N>.<ext>` and removes the scratch source. Publication stages bytes in a unique temporary directory beside `Sources/Images/`, outside the image folder, then uses a same-filesystem operation so an interrupted transfer cannot expose a partial figure or destroy the previous good one. New files use an exclusive hard link; explicit replacements use an atomic rename. If the filesystem cannot support safe publication, the command fails and leaves existing files alone. **An occupied `<image_slug>_fig_<N>.*` slot is refused, not overwritten**: a wrong `--start` would otherwise destroy an existing figure. On the deliberate reprocess path, `--overwrite` permits replacement of the same filename. If the downloaded or rendered format has changed, use the next free number and update the embed; the command preserves the old file and refuses to leave two extensions at one figure number. Rename applies the same occupied-slot rule across extensions and checks both source and destination PDF ownership.
