...
- *Inline `<svg>` diagrams.* You may best-effort serialize a **self-contained** `<svg>` to a unique scratch file outside the vault, then publish it with `fetch_images.py place --attachments '<vault>/Sources/Images' --slug '<slug>' --index <N> --from-file '<scratch>/diagram.svg'` and embed the returned filename. This uses the same ownership, occupied-slot and containment guards as every other recovered image; do not write the serialized markup directly into `Sources/Images/`. If it depends on external CSS/fonts/JS or `<use href="#…">` defs outside the serialized SVG (so it won't render on its own), **don't save a broken file.** Leave a flagged placeholder at the location (`<!-- source has an inline SVG diagram here, not capturable as a static file; view at <source> -->`) and list it in the report. (Note: an `<svg>` or `.svg` that is a *Lottie's static fallback* is not handled here at all — sub-part 1 associated it with its animation and sub-part 3b converts the Lottie to GIF instead. This bullet is only for SVG diagrams that have no Lottie behind them.)
