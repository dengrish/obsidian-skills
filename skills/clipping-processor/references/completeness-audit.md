...
- Fetching: static-first, Playwright on sparse signal
...
3. **Upgrade by re-fetching with Playwright** (same Chromium toolchain sub-part 3b sets up — detect-first, so usually no new dependency): launch the browser, `goto(url, wait_until="networkidle", timeout=30000)`, then read `page.content()` as the audit's raw HTML for sub-part 1 onward. Do this **once** per audit, not per element. Note in the report which path was used (`audit fetched via Playwright (sparse static fetch)` or `audit fetched via curl`) so a future regression is visible.
...
- **Toolchain — render with headless Chromium driving lottie-web** (the canonical Lottie reference renderer). This isn't a preference; it's the only engine that renders Lottie *text layers*. These animations bake their axis labels, numbers, and annotations in as text (Quanta's carry an embedded glyph table plus a "Pangram" font), and every browser-free renderer drops them: `rlottie-python` runs but silently omits all text, and `python-lottie`/`cairosvg` renders the labels blank because its cairo path doesn't use the embedded glyphs. (Note: `python-lottie` ≥ 0.7.2 no longer *crashes* on text layers — an older version raised `'TextDocument' has no attribute 'color'` — but blank labels are just as useless, so the conclusion is unchanged.) A topology number line with no numbers is worse than no figure, so lottie-web it is. The full recipe is **embedded inline below** (written to a temp file at runtime) rather than shipped as a sidecar file, because the skill install doesn't reliably carry sidecar files along with `SKILL.md`.
  - **Get a working browser — detect first, download only if needed.** This is the step that wrongly failed before: the old flow downloaded ~600 MB of Chromium *up front* and, when that hit a sandbox disk limit, bailed to a placeholder **without ever checking for a browser that was already there**. Most sandboxes ship one. Do it in this order:
    1. **Install the Python bindings** — small (~50 MB), always needed to drive the browser. Environments differ in what pip allows, so use the fallback chain (macOS has no bare `pip` command, its stock pip predates `--break-system-packages`, and Debian/Homebrew Pythons refuse an unflagged install as externally managed):
       ```bash
       python3 -m pip install playwright pillow \
           || python3 -m pip install --user playwright pillow \
           || python3 -m pip install --break-system-packages playwright pillow
       ```
    2. **Try to launch with no browser download** (this *is* the probe). Attempt a quick `sync_playwright().chromium.launch()`, render one throwaway frame, close. Sandboxes frequently ship Chromium pre-provisioned — `PLAYWRIGHT_BROWSERS_PATH` pointing at an already-populated dir (e.g. `/opt/pw-browsers`), or a system Chrome — and Playwright finds it automatically (`p.chromium.executable_path` shows what it resolved). **If it launches, you already have a browser: skip every download and go straight to converting.**
    3. **Only if that launch fails, download the smallest sufficient build — and gate it on measured free space, not a vibe.** Prefer the headless shell (`python3 -m playwright install chromium --only-shell`) over full Chromium (`python3 -m playwright install chromium`); the shell renders Lottie identically and `launch()` uses it automatically. **Put the browser on the roomiest mount.** The default install path (`~/.cache/ms-playwright`) may sit on a filesystem that's full even when another mount has room — a real failure mode (e.g. `/sessions` at 100% while `/tmp` has space). Before installing, compare free space across the writable mounts (`df -m /tmp "$HOME" /var/tmp .`), pick the roomiest, and point the installer at it: `export PLAYWRIGHT_BROWSERS_PATH='<roomiest>/pw-browsers'` (verified: the install lands there and `launch()` resolves it automatically). **Concrete disk numbers** (measured, so you can decide deterministically instead of guessing "too tight"): the shell's final footprint is ~330 MB, but during install the ~100 MB download zip briefly coexists with the extracting browser, so the **true peak is ~430 MB** (full Chromium is ~600 MB final / ~760 MB peak — avoid it when space is tight). So: check free space on the chosen mount with `df -m`. **Before deciding it won't fit, reclaim the obvious transient space** — `pip cache purge`, remove any stale `/tmp/*.zip` and old `~/.cache/ms-playwright` partials — which typically frees tens of MB. Then apply a simple rule on the chosen mount: **free ≥ ~550 MB → install the shell (≈430 MB peak + comfortable margin); ~430–550 MB → still attempt the shell (it fits, margin is slim); < ~430 MB → don't attempt** (the shell genuinely can't extract — degrade gracefully, don't risk a half-written browser or filling the disk mid-run). **Never `--with-deps`** — it shells out to `apt-get`, needs root, and aborts the whole install in a non-root sandbox (the original false "Chromium unavailable" bug); the base image almost always already has the system libs. Only retry *with* `--with-deps` if a re-probe fails *specifically* on a missing shared library (not a disk/network error) and you have root.
    4. **Re-probe** — launch once more; if it now works, convert.
    - **If you still can't get a browser**, degrade per the fallback chain below and make the reason **specific and actionable**, citing the real figures — for disk, run `df` and state both sides (e.g. "492 MB free, shell install needs ~430 MB peak"), so the call is auditable; also distinguish download-blocked (network), missing shared lib, and "chromium not found" from the disk case; never a generic "not available." A disk-deferred Lottie isn't lost: the placeholder records its source URL, so the conversion can be **re-run later in a roomier sandbox** (or by hand with the heredoc converter below, re-written to a temp file) — which is exactly how a space-constrained batch run gets its animations back.
- **Convert in three stages, and only the middle one is the heredoc below.** The download and the final write into the vault both go through `scripts/fetch_images.py`; the heredoc renders, and touches neither the network nor `Sources/Images/`. That split is not tidiness. The converter used to do all three itself, and the two it should not have been doing came with none of step 6's guards: it fetched the Lottie URL — text lifted straight out of the page being clipped — with a bare `urlopen`, so no scheme allowlist (`file:` and `ftp:` both work there), no refusal of a host resolving to loopback or `169.254.169.254`, no re-check at a redirect hop, no size cap and no wall-clock cap; and it wrote its output with `os.replace()` straight into `Sources/Images/`, so no slug validation, no path containment, and no refusal of an occupied slot — a pre-seeded GIF from a previous run was overwritten and the script exited 0. `Sources/Images/` is flat and shared with `pdf-figure-extractor` and `paper-summarizer` (`CONVENTIONS.md` §8), so the file it replaced was not necessarily this note's.
  1. **Fetch the source** with `fetch_images.py fetch`, which applies exactly the transport guards step 6's downloads get, and writes to a temp path **outside** the vault (a Lottie is JSON, not an image, so `download` will not carry it — that is what `fetch` is for):
     ```bash
     python3 '<skill>/scripts/fetch_images.py' fetch '<lottie_src .json/.lottie URL>' --out /tmp/lottie_src
     ```
  ...
  3. **Place the GIF** with `fetch_images.py place`, which is step 6's write path: it validates the slug, checks the destination is really inside `Sources/Images/`, **refuses an occupied `<slug>_fig_<N>.*` slot** rather than overwriting it, and picks the extension from the file's own bytes — so a blank or failed render named `.gif` is refused instead of embedded:
     ...
     python3 '<skill>/scripts/fetch_images.py' place --attachments '<vault>/Sources/Images' \
         --slug '<image_slug>' --index <N> --from-file /tmp/lottie_render.gif
     ```
...
```bash
cat > /tmp/lottie_to_gif.py <<'PYEOF'
...
```
  ...
  ```bash
  python3 /tmp/lottie_to_gif.py '<lottie_src .json/.lottie path>' /tmp/lottie_render.gif
  ```
