...
- **`no-source`** — the **raw itself** has no usable HTTP(S) `source:` in its frontmatter: it is missing, malformed, or non-web. **Do not fall through and treat it as `new`.** A note written from it would be invisible to future duplicate checks. Try to recover the URL (a canonical/permalink line in the raw's body, an `og:url`, a URL in the raw's filename), re-check it with `dedup_index.py '<vault>/Articles' --url '<recovered>'` — single quotes, because the recovered URL came off the page (`CONVENTIONS.md` §1b) — and proceed only on `new`. If no URL is recoverable, **skip the raw and report it** in batch mode, or ask the user for the source URL in explicit-single-file mode. A clip whose source is a local PDF follows the PDF routing rules above.
...
On a **reprocess** the body already holds `![[…]]` embeds — never re-download those. If the slug changed, prepare the image rename with `--dry-run` (`fetch_images.py rename --attachments … --sources '<vault>/Sources/PDFs' --old-slug … --new-slug … --dry-run`) and rewrite the draft's embeds. Keep the live image names unchanged until the complete note is ready for finalization under step 11's reprocess procedure.
...
Assemble the complete note at a unique scratch path outside the vault first.
Complete steps 12 and 13 on this draft before publishing it in `Articles/`;
an unfinished note there would enter the next dedup scan. Use this order, with
no blank line between the YAML and the Summary callout:
...
For a new note, publish only the finished, reviewed bytes. Stage them in a unique temporary directory beside `Articles/`, then use an exclusive operation such as `os.link(staged_path, final_path)`; an occupied destination, including a dangling symlink, must fail without changing it. If exclusive publication is unavailable, stop and report the limitation. A prior existence check followed by ordinary `mv` or `os.replace` does not protect a fresh destination from an intervening file.
...
**Collisions and reprocess renames.** If `Articles/<slug>.md` already exists, or a reprocess changes the note's slug, read `references/duplicates-and-reprocessing.md` before publication. It gives the destination preflight, exclusive publication and rollback order. Preserve the original note until the finished replacement can be published. For a renamed clipping, stage the complete new note and preflight its destination first, then apply the reviewed image plan (`fetch_images.py rename --attachments '<vault>/Sources/Images' --sources '<vault>/Sources/PDFs' --old-slug '<old slug>' --new-slug '<new slug>'`), then publish the note. A failed publication requires restoring the old image names. **If you reach step 11 with a collision step 3 did not catch, go back to step 3** — do not rename figures you did not write. A fresh clipping's note and images use the final slug settled at step 3. **Where a write lands on an existing note, its `read:` value is preserved unchanged** (step 10). After publication, read the note back and confirm the embeds resolve before reporting completion.
...
**Decides:** whether the completed draft conforms. **Produces:** fixes before publication, plus flags for the report.

Read the complete scratch draft back from disk and do a structural/format conformance pass against the rules from steps 1–12 — a quick scan, not a re-run of the workflow. Run the mechanical sweep **first** (it makes the machine-detectable failures impossible to skim past), then the source-anchored structural diff, then walk the checklist for the judgment items no grep can make. Mechanical issues that are clearly wrong get fixed before publication; issues that depend on what the user wants get flagged. Then finalize under step 11 and read the published note back. Every fix and every flag is logged in step 14's report, so nothing happens silently.
