...
| `python3 '<skill>/scripts/vault_index.py' '<wiki-folder>'` | step 1 with `--source` for prior coverage; step 3 for collisions; again after the missed-entity audit's creations, before the orphan sweep | `ls`, which cannot see inside files |
...
`vault_index.py` is the load-bearing one. The previously-processed check, collision probes and orphan-link audit need to read *inside* every entry, and one index serves all three. Build it at step 1 when the wiki exists and refresh it at step 3 before collision checks; item 18's vault-wide alias-collision check remains `lint_entry.py`'s own walk. Keep the index **under a unique filename**, as in the snippets below. A fixed `/tmp/vault-index.json` gets silently clobbered by anything else running, and `find_collisions.py` will then return verdicts computed against a different vault, which is indistinguishable from a correct answer.
...
Refresh the index, then probe every candidate. If accepted candidates need a new wiki, create its empty folder inside the already selected vault before indexing it:
