...
**If every source in the run is skipped** — previously processed with no re-run/resume intent, or no durable content under step 2(c) — nothing is created and nothing is merged, so all step-7 audits have empty scope and the run is a genuine run-level no-op. Report the skips and stop. On an authorized resume, this skill's audits cover every entry the resumed run creates or touches; inherited source-independent defects elsewhere in the vault remain wiki-linter's scope.
