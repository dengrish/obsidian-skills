...
- **Wiki refactoring is out of scope for this skill** — partitioning the current source into atomic candidates *before creation* is normal extraction; splitting a pre-existing file is refactoring. wiki-builder does not retype an existing entry, rename slugs and update every wikilink that points at them, redistribute a mixed entry into multiple files after the fact, or merge two existing entries into one. Route an explicit source-backed refactor request to wiki-linter's refactor mode, which inventories the affected vault-wide reference surfaces before writing. Refactoring never occurs as a side effect of processing a source. **The orphan-link audit (step 7) is not a refactoring exception** — it only repairs this run's dangling wikilinks (creating the missed full entry, or unlinking to bare text). It does not rename, split, merge, or rewrite existing entries.
