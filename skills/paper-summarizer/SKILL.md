...
- **It lists nothing → the text names no numbered figures.** Inspect the pages before concluding there are none: image-only pages, unnumbered exhibits and non-English captions do not match this search. If figures are visible, run `pdf-figure-extractor` at this step and report any extraction gaps. Write without embeds only when the page inspection confirms a figureless paper or extraction cannot recover the needed image.
...
**Explain a number without changing what it measures.** A hazard ratio of 0.62 describes a 38% lower estimated event hazard, not 38% fewer events. Hazard is the instantaneous event rate among people still at risk. Give the paper's absolute risks with their follow-up time, or say they are not reported; never derive event counts from the ratio alone. `references/summary-standards.md` distinguishes the common measures.
...
It checks the thirteen rules above — including soft-wrapped sentences and paragraphs — plus the frontmatter's value shapes: `published` a full `YYYY-MM-DD`, `description` within 110 characters, `tags` double-quoted and inside the §3 enum, and `read` a bare boolean (`false` on creation, preserved on rewrites). It also checks that figures belong to this PDF's stem and page numbers start at 1. It exits **1** with one line per violation. Pass `--images '<vault>/Sources/Images'` to confirm every embed exists. A clean lint result does not verify image contents, page upper bounds or factual accuracy; step 10 must still inspect the source.
