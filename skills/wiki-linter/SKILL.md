...
The linter does not set `created:` or `updated:`; invalid dates go to the user. It does not reset, infer, or invent review state. Only `item2/read-type` with a recognizable boolean meaning is a format repair: for example, quoted `"false"` becomes bare `false`. Missing, null, arbitrary-string, and list-valued `read:` stay unchanged and are reported. See [QC field handling](references/qc-items.md#source-independent-review-fix-or-route-per-finding-actions) before repairing metadata.
...
Use the selected paths, keep the output filename unique to this run, and retain it for subsequent slices. A fixed shared temporary filename can supply another vault's results. The image directory must exist; an invalid path is a usage error, not evidence that every figure is missing. Treat `hierarchy_diagnostic.placement_gaps`, `placed_unparented`, `unresolved_parents`, and `moc_marker_states` as report-only evidence from the previously written hierarchy: a fresh builder note normally has a gap until Task 3 runs; `placed_unparented` is the gap list's backward-compatible slug projection; legacy-unmarked or malformed MOCs follow the approval gate; and an unreadable MOC blocks its whole connected closure until the file is readable.
...
The scanner is a **floor, not the whole audit**. Read entries for source-independent semantic checks such as type, introduced aliases, stacked-body problems, main-claim-first openings, paragraph unity and progression, bullets that fragment sequential prose, sentence clarity, mixed-scope split candidates, cross-entry redundancy, media/table placement and captions, and equations stated in their own prose. Keep a sorted coverage ledger for the requested full-entry scope and report how many readable full entries received this semantic pass, naming every skipped or unreadable file; scanner execution alone is never counted as semantic review. **For every ambiguous taxon-shaped, rank-marked, or genus-only `Organism` title, determine the scientific/common-name typography from the cited material; the scanner intentionally emits no recurring problem once the visible title matches.** For paragraph flow, give each paragraph a short phrase naming its purpose, check that every sentence advances its controlling idea, and read the openings in sequence; an abrupt shift is a specific note-improvement proposal, never an automatic body rewrite. Within the requested full-entry scope, compare closely related notes for the same worked example, mechanism walkthrough, or multi-sentence explanation of a neighbor. Name the most specific canonical owner and the passages that should become concise relationships plus links; brief reciprocal contrasts may remain. Verify that the opening states the entity's main claim rather than a lead-in, that list items are genuinely parallel rather than consecutive steps, and that syntax does not obscure the relationship among claims. Atomicity, paragraph coherence, and redundancy are not inferred from length, headings, lexical overlap, or transition words. A split, reorganization, prose conversion, clarity rewrite, or redistribution of legacy content is proposal-only because moving or recasting body content needs its sources and a deliberate editing pass. Existing images and tables can still be checked for source-independent placement and caption rules; source figure selection, table/source-value fidelity, and source fact-checking cannot be completed without the source, so report those needs rather than inventing repairs.
