...
The scanner is a **floor, not the whole audit**. Read entries for source-independent semantic checks such as type, introduced aliases, stacked-body problems, paragraph unity and progression, mixed-scope split candidates, and equations stated in their own prose. For paragraph flow, give each paragraph a short phrase naming its purpose, check that every sentence advances its controlling idea, and read the openings in sequence; an abrupt shift is a specific note-improvement proposal, never an automatic body rewrite. Atomicity and paragraph coherence are not inferred from length, headings, lexical overlap, or transition words. A split or reorganization is proposal-only because moving body content needs its sources and a deliberate editing pass. Source figure selection and source fact-checking cannot be completed without the source; report those needs rather than inventing repairs.
