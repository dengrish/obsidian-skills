...
The downstream consumers do not repair a bare file rename. **The rename must carry these references with it.** The approved `rename_all` workflow below performs that repair; its reference check and authorization rule still apply.
...
implied clean. Default figure ownership and review sidecars in `Sources/Images/`
are tracked separately in `edits.sidecars`; they are shown in the plan and
...
1. **Check that it is a PDF, then check the filename and location.** A file of any other type is out of scope: say so, name it, and stop — do not rename it or move it. A canonical PDF already under `Sources/PDFs/` (or outside the vault) skips steps 2–5. A canonical PDF still in `Inbox/` needs filing: keep its basename, skip metadata extraction in step 2, and follow steps 3–5 with that unchanged basename and `dest='<vault>/Sources/PDFs'`. A correct name alone does not mean the file has reached the folder every downstream skill scans. Continue to step 6 in either case. If the user explicitly requests correction of a canonical filename, use the normal reference check and rename workflow rather than asking them to rename it to a generic name first.
...
2. **Skip canonical files already in `Sources/PDFs/`, but file canonical Inbox PDFs.** Keep an Inbox PDF's existing basename and run the reference check plus `rename_all(..., new_basename=<current basename>, dest='<vault>/Sources/PDFs')`; there is no metadata rename to invent, but the move is still needed. Outside a vault, an already-canonical file stays where it is. Also skip, whole, any subfolder that looks like a book-split output — a folder whose name matches the standalone pattern and which holds chapter-pattern files. Its contents are already canonical, and a book's chapters are the last thing that should be re-derived.
